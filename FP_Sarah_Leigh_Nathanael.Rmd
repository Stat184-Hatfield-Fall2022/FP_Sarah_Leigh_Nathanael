...
```{r top3_enrollment_seperate, echo=TRUE}
# Each Branch Campus Individually
top3_enroll <- ggplot(enrollment_data, aes(FallYear)) + 
  geom_line(aes(y = Abington, color = 'red')) +
  geom_line(aes(y = Erie), color = 'green') +
  geom_line(aes(y = Harrisburg, color = 'blue'))
top3_enroll
```

```{r top3_enrollment_total}
# Total enrollments
...
```{r packageSetup, include = TRUE, echo=TRUE}
#Calling all packages needed to create tables of our data
...
CleryData3 <- read_excel("~/CleryData3.xlsx")
...
CleryDataOtherCampuses <- read_excel("~/CleryDataOtherCampuses.xlsx")
